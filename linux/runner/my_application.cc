...
// GDK window filter on the FlView's GdkWindow. Intercepts button presses
// before Flutter sees them, so we can initiate window drag with the correct
// event serial (required by Wayland xdg_toplevel_move).
static GdkFilterReturn view_filter(GdkXEvent* xevent, GdkEvent* event,
                                   gpointer user_data) {
  if (event->type == GDK_BUTTON_PRESS) {
    GdkEventButton* bev = (GdkEventButton*)event;
    // Top 36 px is the custom Flutter title bar.
    if (bev->y >= 0 && bev->y <= 36) {
      GtkWindow* win = GTK_WINDOW(user_data);
      // The rightmost ~80 px are the window-control buttons; let those reach
      // Flutter so minimize/close still work.
      // We cannot use gtk_widget_get_allocated_width here because we don't have
      // the widget pointer in the filter. Instead, restrict the drag to the
      // left part of the title bar using a reasonable default (the window is
      // 1280 px wide, and the buttons are ~80 px). The filter sees events in
      // FlView's coordinate space, so bev->x is relative to the view.
      if (bev->x < 1200) {
...
static void my_application_activate(GApplication* application) {
  ...
  // Force GTK3 CSD mode on all platforms by setting an invisible zero-height
  // header bar. This prevents the compositor/WM from drawing its own title bar,
  // which works reliably on X11 (GNOME, KDE), Wayland (GNOME, KDE, Sway), and
  // XWayland — unlike gtk_window_set_decorated(FALSE), which is X11-only and
  // ignored by some WMs (e.g. KWin on Wayland ignores X11 decoration requests).
  ...
  // Install a GDK event filter on the FlView's GdkWindow to intercept
  // title-bar button presses before they reach Flutter. This preserves the
  // Wayland serial for gtk_window_begin_move_drag.
  GdkWindow* gdk_window = gtk_widget_get_window(GTK_WIDGET(view));
  if (gdk_window != nullptr) {
    gdk_window_add_filter(gdk_window, view_filter, window);
  }
