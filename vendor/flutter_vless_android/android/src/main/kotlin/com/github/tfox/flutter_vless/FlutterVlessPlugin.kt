...
class FlutterVlessPlugin : FlutterPlugin, ActivityAware, PluginRegistry.ActivityResultListener, MethodChannel.MethodCallHandler {
    ...
    // A single-thread executor serializes every getServerDelay call: with one
    // core spawned per server a sweep of N servers took N * ~1.3s. A small pool
    // lets the app ping many servers concurrently (memory-per-core still limits
    // how big this can be on low-end devices).
