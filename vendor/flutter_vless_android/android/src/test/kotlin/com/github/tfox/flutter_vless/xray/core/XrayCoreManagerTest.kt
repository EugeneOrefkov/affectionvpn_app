...
class XrayCoreManagerTest {
    ...
    @Test
    fun buildRuntimeConfigJson_normalizesFlatVlessWithUsersArrayAndStringPort() {
        val filesDir = File("build/test-files/android-normalize-users")
        val config = XrayConfig(
            V2RAY_FULL_JSON_CONFIG = """
                {
                  "log": {
        ...
        val outbound = output
            ...
            .getJSONArray("vnext")
            .getJSONObject(0)
        val user = outbound.getJSONArray("users").getJSONObject(0)

        assertEquals("universal.example.com", outbound.getString("address"))
        assertEquals(8443, outbound.getInt("port"))
        assertEquals("b94da146-a56e-49d7-af4c-a68c9065cbfd", user.getString("id"))
        assertEquals("none", user.getString("encryption"))
        assertEquals("xtls-rprx-vision", user.getString("flow"))
    }
